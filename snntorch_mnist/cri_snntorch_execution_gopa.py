...
def run_CRI_sim(inputList):
    ...
    for currInput in inputList:
        ...
        for slice in currInput:
            ...
            #breakpoint()
            swPot, swSpike = softwareNetwork.step(slice, membranePotential=True)
            print(len(swPot))
            swPotSet = swPotSet + [(i,element[0],element[1]) for element in swPot]
            #breakpoint()
            #spiked = [element[0] for element in swPot if element[1]>threshold]
            end_time = time.time()
    ...
    print(len(swPotSet))
    swPotSet = set(swPotSet)
    print(i)
    return swPotSet, predictions
...
def run_CRI_hw(inputList):
    ...
    for currInput in inputList:
        ...
        for slice in currInput:
            ...
            start_time = time.time()
            #breakpoint()
            hwPot, hwSpike = hardwareNetwork.step(slice, membranePotential=True)
            print(len(hwPot))
            hwPotSet = hwPotSet + [(i,element[0],element[1]) for element in hwPot]
            #breakpoint()
            #spiked = [element[0] for element in hwPotSet if element[1]>threshold]
            #hwSpike = (spiked,hwSpike[1],hwSpike[2])
            end_time = time.time()
            total_time_cri = total_time_cri + end_time-start_time
            #total_time_cri = total_time_cri + total_time_perstep
            print(hwSpike)
            for spike in hwSpike[0]:
                spikeIdx = int(spike) - firstOutput
                if spikeIdx >= 0:
                    spikeRate[spikeIdx] += 1
    ...
    print(len(hwPotSet))
    print(i)
    return set(hwPotSet), predictions
...
test_loader = DataLoader(mnist_test, batch_size=32, shuffle=True, drop_last=False)
batch_count = 0
with torch.no_grad():
    net.eval()
    for data, targets in iter(test_loader):
        ...
        #breakpoint()
        swPotSet, criPred_sim = run_CRI_sim(input)
        criPred_sim = torch.tensor(criPred_sim).to(device)
        hwPotSet, criPred_hw = run_CRI_hw(input)
        criPred_hw = torch.tensor(criPred_hw).to(device)
        breakpoint()
        ...
        if batch_count == 2: break #run for 10 batches
