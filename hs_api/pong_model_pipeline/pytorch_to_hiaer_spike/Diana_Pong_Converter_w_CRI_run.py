...
import sys
sys.path.insert(0, os.path.join(current_dir, '..', 'ann_training'))
...
# Initialize cumulative counters for the entire run
cumulative_clock_cycles = 0
cumulative_hbm_accesses = 0
...
for episode in range(2):
    ...
    while step_count < max_steps:
        ...
        # 20 cycles with input spikes
        for i in range(20):
            spike_list, cc, hbm = network.step(spikes)
            cumulative_clock_cycles += cc
            cumulative_hbm_accesses += hbm
            if i >= 3:  # Start reading after 3 cycles
                q_values_raw = network.read_membrane(outputs)
                # Handle different output formats from CRI network
                q_values = []
                for q in q_values_raw:
                    if isinstance(q, tuple):
                        # If it's a tuple, take the second element (membrane potential)
                        # CRI read_membrane returns (neuron_key, membrane_potential)
                        q_values.append(float(q[1]) if q[1] is not None else 0.0)
                    elif hasattr(q, 'val'):
                        q_values.append(float(q.val))
                    else:
        ...
        for j in range(4):
            spike_list, cc, hbm = network.step(blank_spikes)
            cumulative_clock_cycles += cc
            cumulative_hbm_accesses += hbm
            q_values_raw = network.read_membrane(outputs)
            # Handle different output formats from CRI network
        ...
        clock_cycles.append((cumulative_clock_cycles, cumulative_hbm_accesses))
