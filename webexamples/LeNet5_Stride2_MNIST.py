"""
LeNet5 with Stride-2 Convolutions on MNIST
============================================
This example demonstrates running inference on the MNIST test set using LeNet5,
a convolutional spiking neural network with stride-2 convolutions, deployed on
HiAER-Spike neuromorphic hardware.

The network processes flattened 28x28 MNIST images as binary spike trains over
7 timesteps. Predictions are made by selecting the output neuron with the
...
def main():
    ...
    for img, label in zip(test_batch['images'], test_batch['labels']):
        ...
        # Run for 7 timesteps through the network
        _ = network.step(inputs)  # Conv1 timesteps
        _ = network.step([])
        _ = network.step([])      # Conv2 timesteps
        _ = network.step([])
        _ = network.step([])      # FC1 timestep
