...
def main():
    ...
    with torch.no_grad():
        for img, label in test_loader:
            ...
            out_hard = torch.tensor(
                cn.run_CRI_hw(cri_input, hardwareNetwork), dtype=float
            ).to(device)
            out_fr = torch.tensor(
                cn.run_CRI_sw(cri_input, softwareNetwork), dtype=float
            ).to(device)

            print(
                f"Label : {label} Soft: {out_fr} Hard: {out_hard} Torch_Pred: {out_tor}"
            )
