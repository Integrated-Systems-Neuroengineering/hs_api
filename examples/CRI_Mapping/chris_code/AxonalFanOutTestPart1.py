...
numberAxons = 1
numberN1 = 1
numberN2 = 4095 #max without binwidth issue
numberN3 = 1
...
#creating all N2 neurons and connect to N3 neurons
for i in range(numberN2):
    for j in range(numberN3):
        connections[f"N.3.{j}"] = ([], N)  #create N3 neuron
        connections[f"N.2.{i}"][0].append((f"N.3.{j}", weightN2_N3)) #connect N2 --> N3
...
neurons_to_read.append(f"N.2.0")
neurons_to_read.append(f"N.3.0")
