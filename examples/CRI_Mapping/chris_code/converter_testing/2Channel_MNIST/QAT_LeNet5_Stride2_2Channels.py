...
class LeNet5(nn.Module):  #class inherits fromm nn.Module, the base class for all Pytorch models
    def __init__(self, num_classes):  #constructor initializes layers & takes num_classes as input 
        ...
        self.conv1 = QuantConv(2, 6, kernel_size=5, stride=2, padding=0, num_bits=16) # 12 12x12 feature maps
        ...
        self.fc1 = QuantLinear(6400, 120, num_bits=16)
...
def main():
  ...
  for epoch in range(num_epochs):
      ...
      if val_losses[-1] < best_val_loss:   #index -1 returns element at end of val_losses
        ...
        torch.save(model.state_dict(), os.path.join(PATH, 'QAT_LeNet5_weights_Stride2_2Channels'))
  ...
  plt.legend()
  plt.tight_layout()
  plt.savefig(os.path.join(PATH, "loss_graph_QATLeNet5_Stride2.png"), dpi=300)  # Save as an image

  #test phase
  model.load_state_dict(torch.load(os.path.join(PATH, "QAT_LeNet5_weights_Stride2_2Channels"))) #load weights/parametes of best trained model into model
  model.eval()  # Set the model to evaluation mode
