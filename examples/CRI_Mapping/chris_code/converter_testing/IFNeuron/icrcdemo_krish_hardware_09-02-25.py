...
def main():
    ...
    # Save all frames of the first training gesture to output_dir/training_sample
    sample_frames, sample_label = full_train_set[0]
    training_sample_dir = os.path.join(output_dir, "training_sample")
    os.makedirs(training_sample_dir, exist_ok=True)
    # sample_frames: [T, C, H, W]
    for t in range(sample_frames.shape[0]):
        for c in range(sample_frames.shape[1]):
            frame = sample_frames[t, c]
            ...
            plt.imsave(os.path.join(training_sample_dir, f"frame_t{t}_c{c}.png"), frame, cmap="gray")
