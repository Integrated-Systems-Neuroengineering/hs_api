...
import models_krish
from utils import train_DVS_Time, train_DVS_Time_with_plot, sw_comp_DVS, validate_DVS, validate_DVS_HW, test_DVS_Time, infer_cri_params
...
def main():
    ...
    #keli's original model
    spiking_neuron = neuron.IFNode
    v_threshold = 1.0
    net = models_krish.DVSGestureNet(channels=20, spiking_neuron=spiking_neuron, surrogate_function=surrogate.ATan(), detach_reset=True)


    # spiking_neuron = neuron.LIFNode
    # v_threshold = 1.0 #needs to be float
    # net = models_krish.DVSGestureNetLinear(
    #     channels=channels,
    #     #encoder=5,
    ...
    # train
    # max_val_acc, train_loss, val_loss, best_epoch = train_DVS_Time_with_plot(args, net, train_loader, val_loader, device, scaler, save_every=25)
    # print("Training Finished")
    # print("Best epoch: ", best_epoch+1)
    # print("Validation accuracy at best epoch: ", max_val_acc)
    ...
    # if hasattr(spiking_layers[0], 'decay_input'):
    #     print("Decay Input: ", spiking_layers[0].decay_input)
    ...
    # converted_path = os.path.join(output_dir, "converted_model")
    # with open(os.path.join(converted_path, "axon_dict.pkl"), 'rb') as f:
    #     axons = dict(pickle.load(f))
    #     print("axon type: ", type(axons))
    #     print("first axon key: ", list(axons)[0])
    #     print("first axon value: ", list(axons.values())[0]) 
    # with open(os.path.join(converted_path, "neuron_dict.pkl"), 'rb') as f:
    #     neurons = dict(pickle.load(f))
    #     print("neuron type: ", type(neurons))
    #     print("first neuron key: ", list(neurons)[0])
    #     print("first neuron value: ", list(neurons.values())[0])
    # with open(os.path.join(converted_path, "output_neurons.pkl"), 'rb') as f:
    #     outputs = pickle.load(f)
    #     print("output neurons type: ", type(outputs))
    #     print("first output neuron: ", outputs[0])
    ...
    hardwareNetwork.sim_flush(os.path.join(output_dir, 'hardwareNetwork_simulation_dump.txt'))
    # print(f"number of params: {n_parameters}")
    # print(f"Number of axons: {len(axons)}")
    # print(f"Number of neurons: {len(neurons)}")
    # print(f"Number of outputs: {len(outputs)}")
