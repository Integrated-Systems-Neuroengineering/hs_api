...
class synthnet:
    ...
    def __init__(self, numAxons, numNeurons, numOutputs, minWeight, maxWeight, maxFan, neuronModel):
    ...
    def roll_axon(self):
        ...
        neurons = [self.gen_neuron_name(neuron) for neuron in neurons]
    ...
    def gen_neuron_dict(self):
        for i in range(self.numNeurons):
            self.neuronsDict[self.gen_neuron_name(i)] = (self.roll_neuron(), self.neuronModel)
    ...
    def gen_inputs(self):
        ...
        return [
            self.gen_axon_name(axonIdx)
            for axonIdx in random.sample(range(0, self.numAxons), numInputs)
        ]
