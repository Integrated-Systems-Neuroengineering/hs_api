...
class Net(nn.Module):
    def __init__(self, in_channels = 1, out_channels = 1, w = 28, h = 28, spiking_neuron: callable = None, **kwargs):
        ...
        self.linear1 = layer.Linear(out_channels * 13 * 13, out_features= 10, bias = True)
    ...
    with torch.no_grad():
        ...
        for img_idx, data in enumerate(tqdm(test_loader)):
            ...
            for t in range(args.T):
                ...
                swOutput, swSpike  = softwareNetwork.step(cri_input[0][0], membranePotential=True)
                ...
                # Record the spikes 
                if t > 1: # TODO: change to layer number -1 (hardcoded now)  to the layer number - 1
